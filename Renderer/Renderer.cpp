...
RenderObject Renderer::loadModel(const tinygltf::Model &model) {
  //
  // m_imageDescriptorPools.reserve(model.images.size() +
  //                                m_imageDescriptorPools.size());
  //
  // uint32_t newTextureIndex = m_imageDescriptorPools.size();
  ...
  //     break;
  //   default:
  //     throw std::runtime_error("Error image format not supported Loading
  //     gltf");
  //   }
  //   VkImage outputImage;
  ...
  //         VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VMA_MEMORY_USAGE_AUTO, 0,
  //         outputIndexBuffer, outputIndexAllocation);
  //
  ...
  //       model.bufferViews[accesor.bufferView];
  //
  ...
  //       vmaMapMemory(m_vmaAllocator, stagingAllocation, &data);
  //       memcpy(data,
  //              dataBuffer.data() + bufferView.byteOffset +
  //              accesor.byteOffset, (size_t)bufferSize);
  //       vmaUnmapMemory(m_vmaAllocator, stagingAllocation);
  //
  ...
  //   }
  //   glm::mat4 initialMatrix = glm::rotate(glm::mat4(1.0f),
  //   glm::radians(90.0f),
  ...
  //   }
  //
  //   nodes.push_back(std::make_shared<Node>(node.name, drawbles,
  //   initialMatrix));
  // }
...
Renderer::loadModel(const Engine::Scene &scene) {
  //
  // std::vector<std::shared_ptr<Drawing>> drawings;
  // m_imageDescriptorPools.reserve(scene.images.size() +
  //                                m_imageDescriptorPools.size());
  // std::vector<std::shared_ptr<VkDescriptorSet>> imageDescriptorSetPtrs;
  // imageDescriptorSetPtrs.reserve(scene.images.size());
  //
  // VkDescriptorPoolSize sceneImageDescriptorSizes{};
  // sceneImageDescriptorSizes.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  // sceneImageDescriptorSizes.descriptorCount =
  //     scene.images.size() * m_imageCount;
  //
  ...
  // sceneImagesDescriptorPool.maxSets = scene.images.size() * m_imageCount;
  ...
  // std::vector<VkVertexInputAttributeDescription>
  //     onlyPosVertexInputAttributeDescriptions;
  // for (auto currentImage : scene.images) {
  //
  //   VkBuffer stagingColorImageBuffer;
  //   VmaAllocation stagingColorImageAllocation;
  //
  ...
  //   }
  //   imageDescriptorSetPtrs.push_back(
  ...
  //   outputImageView; descriptorImageInfo.sampler = outputSampler;
  //
  ...
  // for (const std::shared_ptr<Engine::MeshNode> &node : scene.meshNodes) {
  ...
  //       VertexBufferData{.buffer = meshData.position, .numberOfComponets =
  //       3}, VertexBufferData{.buffer = meshData.normal, .numberOfComponets =
  //       3}, VertexBufferData{.buffer = meshData.texCoord, .numberOfComponets
  //       = 2}, VertexBufferData{.buffer = meshData.tangent, .numberOfComponets
  //       = 3}};
  ...
  //       vmaDestroyBuffer(m_vmaAllocator, stagingBuffer, stagingAllocation);
  //
  //       vulkanBuffers.push_back(VulkanBuffer(outputBuffer,
  //       outputAllocation));
  //
  //       VkVertexInputBindingDescription vertexInputBindingDescription{};
  //       vertexInputBindingDescription.binding = 0;
  ...
  //       case 3: {
  //         vertexInputAttributeDescription.format =
  //         VK_FORMAT_R32G32B32_SFLOAT; break;
  //       }
  //       case 2: {
  //         vertexInputAttributeDescription.format = VK_FORMAT_R32G32_SFLOAT;
  //         break;
  ...
  //   std::shared_ptr<TexturedDrawing> drawing =
  //       std::make_shared<TexturedDrawing>(pipelineTextured, vulkanBuffers,
  //                                         *indexVulkanBufferPtr,
  //                                         imageDescriptorSetPtrs);
...
std::shared_ptr<Drawing> Renderer::loadModel(const Engine::MeshNode &meshNode) {
  ...
  for (size_t i = 0; i < m_imageCount; ++i) {
    ...
    mvpWriteDescriptor.dstSet = *m_mvpDescriptorSets.at(i);
